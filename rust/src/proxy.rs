...
use crate::message::{WSMessage, HttpRequest, HttpResponse, StreamChunk};
use axum::{
...
pub async fn handle_proxy_request(
    ...
    let connection = state.connection_pool.get_connection(&user_id).await
    ...
    let request_message = WSMessage::http_request(
        req_id.clone(),
        method,
        target_url,
        header_map,
...
fn handle_websocket_response(msg: WSMessage) -> Result<Response<String>, StatusCode> {
    match msg.r#type.as_str() {
        "http_response" => {
            if let Some(response) = HttpResponse::from(&msg) {
        ...
        "error" => {
            let error_msg = msg.payload.get("error")
                .and_then(|v| v.as_str())
...
pub async fn handle_streaming_response(
    ...
    let stream = stream::unfold((rx, req_id.clone(), state.clone()), |(mut rx, req_id, state)| async move {
        match rx.recv().await {
            Some(msg) => {
                match msg.r#type.as_str() {
                    "stream_start" => {
                        info!("Stream started for request {}", req_id);
                        Some((axum::response::sse::Event::default().data("stream_start"), (rx, req_id, state)))
                    }
                    "stream_chunk" => {
                        if let Some(chunk) = StreamChunk::from(&msg) {
                            let event = axum::response::sse::Event::default().data(chunk.data);
                            Some((event, (rx, req_id, state)))
                        } else {
                            Some((axum::response::sse::Event::default().data(""), (rx, req_id, state)))
                        }
                    }
                    "stream_end" => {
...
fn authenticate_request(headers: &HeaderMap) -> Result<String, StatusCode> {
    ...
    let expected_key = std::env::var("AUTH_API_KEY")
